<template>
    <div id="MakePageLeftControl">
        <div class="addControl" style="margin-top: 0">
            ...
            <button class="add" @click="$emit('add',{type:'choice',optionsNumber: radioNumber})"
                    :style="{backgroundColor:backgroundColor}">
                {{ $t("makeLeftControlButton") }}
            </button>
        </div>
        <div class="addControl">
            ...
            <br>
            <!--suppress JSUnresolvedVariable -->
            <button class="add" @click="$emit('add',{type:'choice',optionsNumber: choiceNumber})"
                    :style="{backgroundColor:backgroundColor}">{{ $t("makeLeftControlButton") }}
            </button>
        </div>
        <div class="addControl">
            <span class="controlTitle">{{ $t("makeBlank") }}</span><br>
            <button class="add" @click="$emit('add',{type:'blank'})" :style="{backgroundColor:backgroundColor}">
                {{ $t("makeLeftControlButton") }}
            </button>
        </div>
        <div class="addControl">
            <span class="controlTitle">{{ $t("makeManyBlank") }}</span><br/>
            <button class="add" @click="$emit('add',{type:'manyBlank'})" :style="{backgroundColor:backgroundColor}">
                {{ $t("makeLeftControlButton") }}
            </button>
        </div>
        <div class="bottomControl">
            ...
            <button :style="{backgroundColor:backgroundColor}" id="submitRolls" @click="save()">{{ $t("makeSave") }}
            </button>
        </div>
    </div>
</template>
