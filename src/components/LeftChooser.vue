<template>
    <div id="leftChooser">
        <div class="chooser" :style="chooserStyle[0]" @click="$router.push('/queryBigData')">
            <svg
                style="width: 16px;height: 16px;vertical-align: middle;display: inline-block;fill: currentColor;overflow: hidden;"
                viewBox="0 0 1024 1024"
                xmlns="http://www.w3.org/2000/svg">
                <path
                    d="M736.232934 118.192202h-448.922521L62.848129 506.970675 287.309389 895.749148h448.922521l224.46126-388.778473L736.232934 118.192202zM714.634073 858.33945H308.90825l-202.862399-351.368775L308.909274 155.602924h405.725823l202.862399 351.368775L714.634073 858.33945z"
                    fill=""></path>
                <path
                    d="M702.027997 467.579759c-1.821492 0-3.613291 0.121842-5.377446 0.334811l-57.531092-89.131266c9.021454-11.03134 14.444974-25.115907 14.444974-40.445518 0-35.285136-28.706673-63.992833-63.992833-63.992833s-63.992833 28.707697-63.992833 63.992833c0 20.231974 9.448414 38.289216 24.153455 50.023965L432.361176 623.290191a57.620171 57.620171 0 0 0-6.604061-0.392148 57.306862 57.306862 0 0 0-12.382869 1.360744l-56.37615-88.752428a44.318876 44.318876 0 0 0 6.635801-23.387588c0-24.597821-19.941191-44.539012-44.539012-44.539012s-44.539012 19.941191-44.539011 44.539012 19.941191 44.539012 44.539011 44.539011c1.710912 0 3.397252-0.106484 5.057994-0.293855l56.384341 88.764715c-7.465148 9.655239-11.918025 21.757563-11.918025 34.907322 0 31.556146 25.581775 57.137921 57.13792 57.137921s57.137921-25.581775 57.137921-57.137921c0-15.254867-5.989729-29.102916-15.731998-39.349961l119.124962-238.438319a63.816725 63.816725 0 0 0 20.238117-2.214664l57.425632 88.967444a44.329115 44.329115 0 0 0-6.462764 23.117283c0 24.597821 19.941191 44.539012 44.539012 44.539011s44.539012-19.941191 44.539011-44.539011c0-24.596797-19.941191-44.537988-44.539011-44.537988zM564.48641 338.33881c0-13.831667 11.2525-25.08519 25.08519-25.08519s25.08519 11.253524 25.08519 25.08519-11.2525 25.08519-25.08519 25.085191-25.08519-11.253524-25.08519-25.085191z"
                    fill=""></path>
            </svg>
            <span class="chooserText"> {{ $t("queryBigDataOverview") }}</span>
        </div>
        <div class="chooser" style="margin-top: 1px;" :style="chooserStyle[1]" @click="$router.push('/queryDetails')">
            <svg class="icon"
                 style="width:16px;height: 16px;vertical-align: middle;fill: currentColor;overflow: hidden;"
                 viewBox="0 0 1024 1024"
                 xmlns="http://www.w3.org/2000/svg">
                <path
                    d="M959.825022 234.949415C959.825022 140.5022 883.262992 63.940169 788.815776 63.940169H234.949415C140.5022 63.940169 63.940169 140.5022 63.940169 234.949415v553.866361C63.940169 883.262992 140.5022 959.825022 234.949415 959.825022h553.866361c94.443603 0 171.009246-76.562031 171.009246-171.009246V234.949415z m-63.940168 554.039758c0 28.599679-11.050451 55.400363-31.272884 75.622797S817.588852 895.884854 788.989173 895.884854H235.007214c-28.599679 0-55.515961-11.050451-75.738394-31.272884S127.880338 817.588852 127.880338 788.989173V235.007214A107.170226 107.170226 0 0 1 235.007214 127.880338h553.981959c28.599679 0 55.400363 11.166049 75.622797 31.388482S895.884854 206.407535 895.884854 235.007214v553.981959z"
                    fill=""></path>
                <path
                    d="M799.999887 289.356358H224.000113a31.970084 31.970084 0 0 0 0 63.940169h575.999774a31.970084 31.970084 0 0 0 0-63.940169zM799.999887 480.093133H224.000113a31.970084 31.970084 0 1 0 0 63.940169h575.999774a31.970084 31.970084 0 1 0 0-63.940169zM799.999887 670.829908H224.000113a31.970084 31.970084 0 0 0 0 63.940169h575.999774a31.970084 31.970084 0 0 0 0-63.940169z"
                    fill=""></path>
            </svg>
            <span class="chooserText">{{ $t("queryDetails") }}</span>
        </div>
    </div>
</template>

<script>
export default {
    name: "LeftChooser",
    data() {
        return {
            chooserStyle: [
                {backgroundColor: "#f5f5f5"},
                {backgroundColor: "white"}
            ]
        }
    },
    props: {
        mode: String,
    },
    mounted() {
        if (this.mode === "bigData") {
            this.chooserStyle[0].backgroundColor = "white";
            this.chooserStyle[1].backgroundColor = "#f5f5f5";
        } else {
            this.chooserStyle[0].backgroundColor = "#f5f5f5";
            this.chooserStyle[1].backgroundColor = "white";
        }
    }
}
</script>

<style scoped>
#leftChooser {
    position: fixed;
    height: calc(100% - 73px);
    margin-top: 11px;
    top:62px;
    width: 25%;
    background-color: #f5f5f5;
}

.chooser {
    text-align: center;
    height: 20px;
    padding: 20px 0 20px 0;
    width: calc(100% - 1px);
    background: white;
    cursor: pointer;
}
</style>